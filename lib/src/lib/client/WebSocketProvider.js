...
const WebSocketProvider = ({ id, url, domain, ping, logging, reconnect, showElementWhileConnecting, children, }) => {
    ...
    (0, react_1.useEffect)(() => {
        const manager = new WebSocketManager_1.WebSocketManager(url, domain, reconnect, ping, logging || false);
        setManager(manager);
    ...
    const onError = (error, info) => {
        const errorBean = (0, Beans_1.createClientErrorBean)({
            message: "" + error,
            componentStack: info === null || info === void 0 ? void 0 : info.componentStack,
        });
        if (manager)
            manager.send((0, IOClientToServerCoreBeanBuilder_1.clientToServerCoreBeanBuilder)()
