...
exports.useRemoteStoreUpdate = (path, params, validationFunction, postServerValidationCallback, postClientValidationCallback) => {
    ...
    const editRemoteBean = (payload) => {
        ...
        if (!params || !params.key) {
            remoteStore.editRemoteStore(StoreBeans_1.CoreMessage.STORE_CREATE, path, params || null, payload, componentId);
        }
        else {
            remoteStore.editRemoteStore(StoreBeans_1.CoreMessage.STORE_EDIT, path, params, payload, componentId);
        }
    };
    react_1.useEffect(() => {
