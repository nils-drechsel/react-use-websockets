...
class RemoteStore {
    ...
    openRemoteStore(path, params) {
        ...
        if (!this.store.has(storeId)) {
            ...
            this.websocketManager.send(StoreBeans_1.CoreMessage.STORE_CONNECT, payload);
        }
    }
    closeRemoteStore(path, params) {
        ...
        this.websocketManager.send(StoreBeans_1.CoreMessage.STORE_DISCONNECT, payload);
    }
    getData(path, params) {
        const storeId = StoreBeanUtils_1.createStoreId(path, params);
        if (!this.store.has(storeId))
    ...
    editRemoteStore(path, params, payload, originId) {
        const bean = {
            path,
            params,
            payload,
        ...
        this.websocketManager.send(StoreBeans_1.CoreMessage.STORE_EDIT, bean);
    }
    update(storeId, data) {
        if (!this.store.has(storeId)) {
