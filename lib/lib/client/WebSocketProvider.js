...
const WebSocketProvider = ({ id, url, domain, delimiter, ping, logging, reconnect, showElementWhileConnecting, children, }) => {
    const managerMap = react_1.useContext(WebSocketContext_1.WebSocketContext);
    const [manager] = react_1.useState(new WebSocketManager_1.WebSocketManager(url, domain, delimiter || "\t", reconnect, ping, logging || false));
    react_1.useEffect(() => {
        return () => {
            manager.destroy();
        };
