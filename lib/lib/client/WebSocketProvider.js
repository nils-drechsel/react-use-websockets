...
exports.WebSocketProvider = ({ id, url, domain, delimiter, logging, reconnect, showElementWhileConnecting, children }) => {
    const managerMap = react_1.useContext(WebSocketContext_1.WebSocketContext);
    const managerRef = react_1.useRef();
    if (!managerRef.current) {
        managerRef.current = new WebSocketManager_1.WebSocketManager(url, domain, delimiter || "\t", reconnect, logging || false);
    }
