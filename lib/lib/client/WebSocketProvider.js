...
const ClientToServerCoreBeanBuilder_1 = require("./ClientToServerCoreBeanBuilder");
const WebSocketContext_1 = require("./WebSocketContext");
const WebSocketManager_1 = require("./WebSocketManager");
const WebSocketProvider = ({ id, url, domain, ping, logging, reconnect, showElementWhileConnecting, children, serialisationSignatures, }) => {
    ...
    (0, react_1.useEffect)(() => {
        const manager = new WebSocketManager_1.WebSocketManager(url, domain, reconnect, ping, logging || false, serialisationSignatures);
        setManager(manager);
    ...
    const [, setUid] = (0, react_1.useState)(null);
    const onError = (error, info) => {
        ...
        if (manager)
            manager.send((0, ClientToServerCoreBeanBuilder_1.clientToServerCoreBeanBuilder)()
                .endpoint(Beans_1.IOCoreEndpoints.CORE)
                .message(Beans_1.CoreMessage.CLIENT_ERROR)
                .payload(errorBean)
                .build());
    };
    const errorFallback = ({}) => {
    ...
    if ((!manager || !isSocketConnected) && showElementWhileConnecting) {
        return react_1.default.createElement("div", null, (0, react_1.cloneElement)(showElementWhileConnecting));
    }
    const map = new Map();
    if (managerMap) {
