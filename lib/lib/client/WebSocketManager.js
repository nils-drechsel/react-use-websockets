...
exports.WebSocketManager = exports.addDeserialisers = exports.addSerialisers = void 0;
...
const addSerialisers = (serialisers, serialisationSignatures, serialisationPairs) => {
    if (serialisationPairs && serialisationSignatures) {
        serialisationPairs.forEach((entity, message) => {
            ...
            if (!signature)
                return;
            serialisers.set(message, new Serialisation_1.default(signature));
        });
    }
};
exports.addSerialisers = addSerialisers;
const addDeserialisers = (deserialisers, serialisationSignatures, deserialisationPairs) => {
    if (deserialisationPairs && serialisationSignatures) {
        deserialisationPairs.forEach((entity, message) => {
            const signature = serialisationSignatures.get(entity);
            if (!signature)
                return;
            deserialisers.set(message, new Deserialisation_1.default(signature));
        });
    }
};
exports.addDeserialisers = addDeserialisers;
class WebSocketManager {
    constructor(url, domain, delimiter = "\t", reconnect = false, ping = 5, logging = true, serialisationSignatures, serialisationPairs, deserialisationPairs) {
        ...
        exports.addSerialisers(this.serialisers, serialisationSignatures, serialisationPairs);
        exports.addDeserialisers(this.deserialisers, serialisationSignatures, deserialisationPairs);
