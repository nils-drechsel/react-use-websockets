...
class WebSocketManager {
    constructor(url, domain, reconnect = false, ping = 5, logging = true, serialisationSignatures) {
        ...
        this.unsubscribeInterval = setInterval(() => {
            if (this.logging)
                console.log("PING");
            this.send((0, ClientToServerCoreBeanBuilder_1.clientToServerCoreBeanBuilder)()
    ...
    onMessage(event) {
        ...
        const bean = (0, ServerToClientCoreBeanBuilder_1.serverToClientCoreBeanBuilder)()
            ...
            .origin((_a = coreBean.origin) !== null && _a !== void 0 ? _a : undefined)
            .fromSid((_b = coreBean.fromSid) !== null && _b !== void 0 ? _b : undefined)
            .build();
        ...
        callbacks === null || callbacks === void 0 ? void 0 : callbacks.forEach((callback) => (callback ? callback(bean) : null));
    }
    resolveQueue() {
    ...
    send(coreBean) {
        ...
        if (!this.isConnected()) {
            this.queue.push(raw);
            if (this.logging)
                console.log("queueing", coreBean.message, "with payload", _payload);
        }
        else {
            if (this.logging)
                console.log("sending", coreBean.message, "with payload", _payload);
            this.sendRaw(raw);
        }
    }
    sendRaw(raw) {
